...
X = sdpvar(4,N);
U = sdpvar(3,N);
nu  = sdpvar(4,N); % slack variables for virtual control
Sigma = sdpvar(1,1);
...
for Index = 1:Max_iter
    ...
    for i = 1:N-1
        ...
        Cons = Cons + [ -H1*X(:,i) + H2*X(:,i+1) - G1*U(:,i) - G2*U(:,i+1) - 0.5*step*(nu(:,i)+nu(:,i+1)) == 0.5*step*(b1+b2) ];  % N
...
%     Objective = J + lambdav*norm(nu,1) +lambdasigma*delta2;
...
%     Cons = Cons + [ -delta <= X-Xk <= delta ];
    ...
    Uk = [u1'; u2'; u3']; % U_k-1
    delta1 = [0.2*U(1,j)*ones(1,N); 0.2*U(2,j)*ones(1,N); 0.2*U(3,j)*ones(1,N)];
    Cons = Cons + [ -delta1 <= U-Uk <= delta1 ];
    
    
    
    delta2 =0.02*sigma_guess ;
    Cons = Cons + [ -delta2 <= Sigma-sigma <= delta2 ];
...
%     end
    ...
    lambdasigma = 1e-1;
    lambdax = 1e-3;
%    Objective = J + lambdav*norm(nu,1) + lambdasigma*norm(delta2,1) + lambdax*norm(delta1,1)+ lambdax*norm(delta,1);
    Objective = J  + lambdav*norm(nu,1) +lambdasigma*norm(delta2,1)+lambdax*norm(delta1,1);

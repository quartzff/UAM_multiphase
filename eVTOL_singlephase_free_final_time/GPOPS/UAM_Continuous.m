...
path2 = u3*sin(thetamin)-u1;
path3 = u1-u3*sin(thetamax);
path4 = u3*cos(thetamax)-u2;
path5 = u2-u3*cos(0);
path6 = u1.^2+u2.^2-u3.^2;
phaseout.path  = [path1, path2, path3, path4, path5, path6];
  
% ---------------------------------------------------%
% ------------ Evaluate Integral Cost -------------- %
% ---------------------------------------------------%
phaseout.integrand = 1/2*u3.^2/1e9; % minimum energy

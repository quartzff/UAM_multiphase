...
%         X(:,i+1) = inv(H2) * (H1*X(:,i) + G1*U(:,i) + G2*U(:,i+1) + 0.5*step*(b1+b2));
        ...
        %J = J + 1/1e9*step*(1/2*sigma*U(3,i+1)^2+u3_02*Sigma);
        %J = J + 1/1e9 * step*(1/2*u3_01^2*sigma)+1/2*u3_01^2*(Sigma - sigma)+U(3,i)*sigma*(U(3,i)-u3_01);%First order linearization
...
%         Cons = Cons + [ norm(U(1:2,j)) <= U(3,j) ];
     ...
     %Cons = Cons + [  Sigma == 1455.627 ];% added time constraint

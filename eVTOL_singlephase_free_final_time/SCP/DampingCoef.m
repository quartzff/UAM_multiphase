function c = DampingCoef(Zk1, Zk2, mu)
global cf lambda

c = 1;
phi_old = MeritFunc(Zk1, mu);
P       = Zk2 - Zk1; % 8*N

%% Backtracking Line Search (Wolfe Conditions)
% (mu = 10, cf = 0.5, lambda = 0.4) works good, most of c is equal to 1
% eps = 1e-6;
% Zk_eps = Zk1 + eps*P;
...
% (mu = 10, cf = 0.5, lambda = 0.4 or 0.2) works OK
% eps = 1e-6;
% Zk_eps = Zk1 + eps*P;
% phi_eps = MeritFunc(Zk_eps, mu);
% D_phi = (phi_eps-phi_old)/eps;
